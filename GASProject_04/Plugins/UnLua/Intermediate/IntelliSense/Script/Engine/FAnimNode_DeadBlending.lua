...
---@field private bLinearlyInterpolateScales boolean @When enabled, bone scales will be linearly interpolated. This is slightly more performant and consistent with the rest of Unreal but visually gives the appearance of the rate of change of scale being affected by the overall size of the bone.
...
---@field private bPreallocateMemory boolean @Enable this to pre-allocate memory for the node rather than to allocate and deallocate memory when blending becomes active and inactive. This improves performance, but causes larger memory usage, in particular when you have multiple Dead Blending nodes in an animation graph that are not all used at once.
