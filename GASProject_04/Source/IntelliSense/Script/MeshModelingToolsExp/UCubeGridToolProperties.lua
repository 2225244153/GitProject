...
---@field public GridPower integer @Determines cube grid scale. Can also be adjusted with hotkeys.
---@field public CurrentBlockSize number @Sets the size of a block at the current grid power. This is done by changing the base block size (i.e. the size at grid power 0) such that the target size is achieved at the the current value of Grid Power.
...
---@field public BlockBaseSize number @Smallest block size to use in the grid. For instance, 3.125 results in        blocks that are 100 sized at 5 power of two since 3.125 * 2^5 = 100.
