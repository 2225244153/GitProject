...
---@field private UniqueIdxToComponent TMap<integer, FClusterUnionParticleCandidateData> @Given a unique index of a particle that we're adding to the cluster union - map it back to the component that owns it. This works decently because we assume that when we're using a cluster union component, we will only try to add to the cluster union via the GT so we can guarantee to have a decent mapping here.
