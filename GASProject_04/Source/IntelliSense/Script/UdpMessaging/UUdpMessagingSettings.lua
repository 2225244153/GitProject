...
---@field public WorkQueueSize integer @The buffer size for the working send queues. Each node connections gets a send queue and new messages to send are put on that queue.  The send scheduler will fairly send data on this queue and re-queue when partial data is sent or waiting for a reliable message. Can be specified on the command line with `-UDPMESSAGING_WORK_QUEUE_SIZE=`
...
---@field public ExcludedEndpoints TArray<string> @List of IP addresses that are banned from communicating with this client. The format is IP_ADDRESS:PORT_NUMBER.  If port number is 0 then all ports are blocked.
---@field public bShareKnownNodesWithActiveConnections boolean @Whether to share our list of known endpoints with those endpoints.  This allows message processors to completely mesh their networks.
